...
description: An explanation of the proof-of-work consensus protocol and its role in Cloiinkcoin.
lang: en
sidebar: true
incomplete: true
---

Cloiinkcoin, like Bitcoin, currently uses a consensus protocol called [Proof-of-work (PoW)](https://en.wikipedia.org/wiki/Proof_of_work). This allows the nodes of the Cloiinkcoin network to agree on the state of all information recorded on the Cloiinkcoin blockchain, and prevents certain kinds of economic attacks.

Over the next few years, proof-of-work will be phased out in favour of [proof of stake](/developers/docs/consensus-mechanisms/pos). This will also phase out mining from Cloiinkcoin. For more details on timing, check the progress of the [Eth2 merge upgrade](/eth2/merge/).
...
Proof of Work (PoW) is the mechanism that allows the decentralized Cloiinkcoin network to come to consensus, or agree on things like account balances and the order of transactions. This prevents users "double spending" their coins and ensures that the Cloiinkcoin chain is incredibly difficult to attack or overwrite.

## Proof-of-work and mining {#pow-and-mining}

Proof-of-work is the underlying algorithm that sets the difficulty and rules for the work miners do. Mining is the "work" itself. It's the act of adding valid blocks to the chain. This is important because the chain's length helps the network spot the valid Cloiinkcoin chain and understand Cloiinkcoin's current state. The more "work" done, the longer the chain, and the higher the block number, the more certain the network can be of the current state of things.

[More on mining](/developers/docs/consensus-mechanisms/pow/mining/)

## How does Cloiinkcoin's proof-of-work work? {#how-it-works}

Cloiinkcoin transactions are processed into blocks. Each block has a:
...
Miners who successfully create a block are rewarded in 2 freshly minted CLK and all the transaction fees within the block. A miner may also get 1.75CLK for an uncle block. This is a valid block, created simultaneously to the successful block, by another miner. This usually happens due to network latency.
...
| Pros                                                                                                                                                                                                               | Cons                                                                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| PoW is neutral. You don't need CLK to get started and block rewards allow you to go from 0CLK to a positive balance. With [proof of stake](/developers/docs/consensus-mechanisms/pos/) you need CLK to start with. | PoW uses up so much energy that it's bad for the environment.                                                                                |
| PoW is a tried and tested consensus mechanism that has kept Bitcoin and Cloiinkcoin secure and decentralized for many years.                                                                                       | If you want to mine, you need such specialized equipment that it's a big investment to start.                                                |
...
- PoS switches out the importance of computational power for staked CLK.
- PoS replaces miners with validators. Validators stake their CLK to activate the ability to create new blocks.
...
- [On settlement finality](https://blog.cloiinkcoin.com/2016/05/09/on-settlement-finality/)
